...
<div class="section">
  ...
  <table>
    ...
    <tbody>${datos.capacitaciones.map(c => `
      <tr><td>${c.nombre}</td><td>${c.categoria||''}</td><td>${c.modalidad||''}</td><td>${c.estado||''}</td><td>${c.horas}h</td><td>₡${Number(c.costo).toLocaleString()}</td></tr>`).join('')}
    </tbody>
  </table>
</div>
<div class="section">
  ...
  <table>
    ...
    <tbody>${datos.participantes.map(p => `
      <tr><td>${p.colaboradores?.nombre||''}</td><td>${p.colaboradores?.correo||''}</td><td>${p.colaboradores?.gerencia||''}</td><td>${p.capacitaciones?.nombre||''}</td><td>${p.capacitaciones?.horas||0}h</td></tr>`).join('')}
    </tbody>
  </table>
</div>
<div class="section">
  ...
  <table>
    ...
    <tbody>${datos.presupuesto.map(m => `
      <tr><td>${m.centro_gestor}</td><td>${m.posicion}</td><td>${m.cd}</td><td>${m.texto}</td><td>₡${Number(m.importe).toLocaleString()}</td></tr>`).join('')}
    </tbody>
  </table>
</div>
<div class="footer">ControlCap · Universidad Corporativa · CoopeAnde N.º 1 · Uso interno exclusivo</div>
</body></html>`
  ...
  async function generarExcel() {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(cap.length ? cap : [{}]), 'Capacitaciones')
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(par.length ? par : [{}]), 'Participantes')
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(col.length ? col : [{}]), 'Colaboradores')
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(pre.length ? pre : [{}]), 'Presupuesto')
