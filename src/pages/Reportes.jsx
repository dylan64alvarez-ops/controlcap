...
export default function Reportes() {
  ...
  async function cargarCatalogos() {
    const [colRes, capRes] = await Promise.all([
      supabase.from('colaboradores').select('id, nombre, correo, gerencia, departamento').order('nombre'),
  ...
  async function obtenerDatosFiltrados() {
    // Capacitaciones filtradas por año y proveedor
    ...
    let qPart = supabase.from('participantes').select('*').in('capacitacion_id', capIds)
...
<div class="footer">
  ...
  async function generarExcel() {
    ...
    try {
      ...
      // Resumen por gerencia (solo si no es reporte por colaborador)
      if (!filtroColab) {
        const porGerencia = {}
        parts.forEach(p => {
          const g = p._gerencia || 'Sin gerencia'
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(
        parts.length ? parts.map(p => ({
          Colaborador: p._nombre,
          Correo: p._correo,
          Género: p.genero || '—',
          Gerencia: p._gerencia,
          Departamento: p._departamento,
          Puesto: p._puesto,
          Capacitación: p._cap?.nombre || '—',
          Proveedor: p._cap?.proveedor || '—',
      ...
      const filtroFile = [
        ...
        filtroGerencia,
        filtroDep,
        filtroProveedor ? `Prov_${filtroProveedor.slice(0,15)}` : '',
        colabSeleccionado ? colabSeleccionado.nombre.split(' ').slice(0,2).join('_') : ''
      ].filter(Boolean).join('_') || 'General'
  ...
  async function generarPPTX() {
    ...
    try {
      ...
      // Slide 3: Por gerencia (solo si no es reporte por colaborador)
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
                ...
                <div style={{ position: 'absolute', top: '40px', left: 0, width: '280px', background: 'white', border: '1px solid #E2E8F0', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 50, maxHeight: '200px', overflowY: 'auto' }}>
                  {resultadosColab.map((c, i) => (
                    <div key={c.id} onClick={() => seleccionarColab(c)}
                      style={{ padding: '8px 12px', cursor: 'pointer', fontSize: '12px', borderBottom: i < resultadosColab.length - 1 ? '1px solid #F1F5F9' : 'none', background: 'white' }}
          ...
          {(filtroAnio || filtroGerencia || filtroDep || filtroCap || filtroColab || filtroProveedor) && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <button onClick={() => {
                setFiltroAnio('2026'); setFiltroGerencia(''); setFiltroDep('')
                setFiltroCap(''); setFiltroProveedor(''); limpiarColab()
              }}
